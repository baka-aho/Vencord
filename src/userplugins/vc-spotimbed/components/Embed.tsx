...
function UnsupportedEmbed() {
    return <div className={cl("unsupported")}>This Spotify embed is not supported by SpotiMbed, please <a href={REPO_ISSUES_URL} target="_blank" rel="noreferrer">open an issue</a> and include the above link</div>;
}
...
export function createSpotimbed(embed: Embed) {
    const url = parseUrl(embed.url);
    if (!url) return null;
...
export function Spotimbed({ art: initialArtUrl, type: resourceType, id: resourceId, tempSettings }: SpotimbedProps) {
    ...
    const [palette, , artPending] = useCachedAwaiter(async () => {
        ...
        const palette = await getPaletteFromUrl(artUrl, 128, 10);
        return palette;
    }, {
