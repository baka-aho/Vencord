...
export async function resolveSpotifyLinks(links: string[], stripQuery = true) {
    const settled = await Promise.allSettled(links.map(link => fetch(link).then(res => res.json() as Promise<{ href: string | null; }>)));
    const hrefs: string[] = [];
    for (const result of settled) {
        ...
        hrefs.push(stripQuery ? href.split("?")[0] : href);
    }
    return hrefs;
}
